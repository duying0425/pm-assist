...
| v1.3.6 | 本地完成 | 认证跳板全局授权 + 配置热加载：HUB_CLIENTS 从 .env 迁至 hub_config.json（gitignore，含模板 example），每次请求现读、改 scope/注册新应用免重启；授权 scope 改为全局一份（191 项，从飞书已开通权限导出裁剪至 200 上限内），接入应用复用 pm-assist 已开通权限时 hub 零改动、存量 token 无需重新授权 |
